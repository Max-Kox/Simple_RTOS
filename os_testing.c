// os_testing.c used to test Real-Time Operating System implemented in files os.c and osasm.s
// Runs on TM4C123
// This testing program program will measure steps using the accelerometer,
// audio sound amplitude using the microphone, temperature using the TMP006,
// and light using the OPT3001, process the data, and output the results.
...
#define THREADFREQ 1000			// frequency in Hz of  scheduler
...
// Main thread scheduled by OS scheduler, accepts data from accelerometer, calculates steps, plots on LCD
...
// *********Task3*********
// Main thread scheduled by OS scheduler, real-time task, signaled on touch
...
// *********Task4*********
// Main thread scheduled by OS scheduler, measures temperature
// Inputs:  none
// Outputs: none
void Task4(void)
{
...
// *********Task5*********
// Main thread scheduled by OS scheduler, updates the text at the top and bottom of the LCD
// Inputs:  none
// Outputs: none
void Task5(void)
{
	...
	while(1)
	{
		...
		BSP_LCD_OutUFix2_1(TemperatureData, TEMPCOLOR);
		BSP_LCD_SetCursor(5,  1);
		BSP_LCD_OutUDec4(Steps,             MAGCOLOR);
		BSP_LCD_SetCursor(16, 0);
		BSP_LCD_OutUDec4(LightData,         LIGHTCOLOR);
		BSP_LCD_SetCursor(16, 1);
		BSP_LCD_OutUDec4(SoundRMS,          SOUNDCOLOR);
		BSP_LCD_SetCursor(16,12);
		BSP_LCD_OutUDec4(Time/10,           TOPNUMCOLOR);
//debug code
		if(LostTask1Data)
		{
			BSP_LCD_SetCursor(0, 12);
			BSP_LCD_OutUDec4(LostTask1Data, BSP_LCD_Color565(255, 0, 0));
		}
//end of debug code
		OS_Signal(&LCDmutex);
	}
...
// *********Task6*********
// Main thread scheduled by OS scheduler. Task6 measures light intensity
// Inputs:  none
// Outputs: none
void Task6(void)
{
...
// *********Task7*********
// Main thread scheduled by OS scheduler. Task7 does nothing but never blocks or sleeps
// Inputs:  none
// Outputs: none
uint32_t Count7;
void Task7(void)
...
// main implements the fitness device by combining the OS functions
